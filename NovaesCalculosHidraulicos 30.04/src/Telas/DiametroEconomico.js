...
const DiametroEconomico = ({ navigation }) => {
  ...
  const adicionarAoHistorico = (resultado) => {
    if (!resultado.includes("Por favor")) { // Verifica se o resultado não contém a mensagem de erro
  ...
  const salvarHistorico = async () => {
    try {
      await AsyncStorage.setItem('@historicoDiametroEconomico', JSON.stringify(historico));
    } catch (error) {
  ...
  const carregarHistorico = async () => {
    try {
      const historico = await AsyncStorage.getItem('@historicoDiametroEconomico');
      if (historico !== null) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.historicoContainer}>
          ...
          {historico.map((item, index) => (
            <Text key={index} style={styles.historicoItem}>{`Resultado ${index + 1}: ${item}`}</Text>
          ))}
        </View>
